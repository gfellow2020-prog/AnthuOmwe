...
<div class="flex items-center justify-between mb-6">
    <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Screening Queue</h1>
        <p class="text-sm text-gray-500 mt-0.5">Initial clinical assessment desk</p>
    </div>
...
<div class="space-y-6">
        ...
        <div class="divide-y divide-gray-100">
            ...
            <div class="flex items-center justify-between px-6 py-4 hover:bg-gray-50 transition">
                ...
                <div class="flex items-center gap-3">
                    ...
                    @endif
                    <a href="{{ route('screening.show', $enc) }}" class="btn-success text-xs px-3 py-1.5">
                        Continue Assessment
                    </a>
                </div>
            </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700">
        <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-700 flex items-center gap-3">
            ...
            <h2 class="text-base font-semibold text-gray-900 dark:text-white">Waiting to be Received</h2>
            <span class="ml-auto text-xs font-semibold bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full">{{ $queued->total() }}</span>
        </div>

        @if($queued->isEmpty())
        <div class="px-6 py-10 text-center text-sm text-gray-400">
            No patients waiting. Patients arrive here after Triage completes their assessment.
        </div>
        @else
        <div class="divide-y divide-gray-100">
            ...
            <div class="flex items-center justify-between px-6 py-4 hover:bg-gray-50 transition">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-500">
                            {{ $enc->encounter_number }}
                            @if($enc->visit_type) · {{ $enc->visit_type }} @endif
                            · Arrived {{ $enc->updated_at->diffForHumans() }}
                        </p>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <span class="badge badge-{{ $enc->priority_level }}">{{ ucfirst($enc->priority_level) }}</span>
                    @endif
                    <form method="POST" action="{{ route('screening.receive', $enc) }}">
                        @csrf
