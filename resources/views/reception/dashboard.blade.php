...
<div class="mb-8 rounded-2xl border border-neutral-200 dark:border-neutral-800 bg-gradient-to-br from-neutral-50 to-neutral-100/70 dark:from-neutral-950/40 dark:to-gray-900 dark:bg-gray-800 shadow-sm overflow-hidden relative">
    <div class="pointer-events-none absolute -left-5 top-10 text-neutral-300/70 dark:text-neutral-700/40">
        <svg class="w-16 h-16" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.78 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z"/></svg>
    </div>
    <div class="pointer-events-none absolute right-2 top-8 text-neutral-300/70 dark:text-neutral-700/40">
    ...
    <div class="relative p-4 md:p-6">
        <div class="flex items-center justify-between mb-4">
            <h2 class="text-2xl font-bold text-neutral-900 dark:text-neutral-100">Welcome Anthu Omwe</h2>
            <p class="text-sm text-slate-500 dark:text-slate-300">{{ now()->format('l, F d, Y') }}</p>
        </div>

        <h3 class="text-center text-3xl font-extrabold tracking-tight text-neutral-900 dark:text-neutral-100 mb-5">Search or Add New Patient</h3>
        ...
        <form method="GET" action="{{ route('reception.dashboard') }}">
            ...
            <div id="barcode-mode-fields" class="{{ $isBarcodeMode ? '' : 'hidden' }}">
                <div class="max-w-3xl mx-auto mb-4 rounded-2xl border border-neutral-200 dark:border-neutral-700 bg-white/80 dark:bg-gray-900/60 p-4">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="space-y-3">
                            ...
                            <input
                                ...
                                class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-neutral-500"
                            >
                            <div class="flex flex-wrap gap-2">
                                <button type="button" onclick="startBarcodeScanner()" class="px-4 py-2 rounded-lg bg-neutral-900 hover:bg-neutral-800 text-white text-sm font-semibold">Start Scanner</button>
            ...
            <div id="single-mode-fields" class="{{ ($isFullNameMode || $isBarcodeMode) ? 'hidden' : '' }}">
                <div class="max-w-xl mx-auto mb-4">
                    <input
                        ...
                        class="w-full px-5 py-3.5 rounded-full border border-slate-200 dark:border-slate-700 bg-white/95 dark:bg-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-neutral-500"
                    >
                </div>
            </div>
            ...
            <div id="full-name-fields" class="{{ $isFullNameMode ? '' : 'hidden' }}">
                ...
                <div class="max-w-3xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2.5 mb-4">
                    <input type="text" name="first_name" value="{{ $firstName }}" placeholder="First name" class="px-4 py-3 rounded-full border border-slate-200 dark:border-slate-700 bg-white dark:bg-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-neutral-500">
                    <input type="text" name="last_name" value="{{ $lastName }}" placeholder="Surname" class="px-4 py-3 rounded-full border border-slate-200 dark:border-slate-700 bg-white dark:bg-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-neutral-500">
                    <input type="date" name="dob" value="{{ $dob }}" class="px-4 py-3 rounded-full border border-slate-200 dark:border-slate-700 bg-white dark:bg-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-neutral-500">
                    <select name="gender" class="px-4 py-3 rounded-full border border-slate-200 dark:border-slate-700 bg-white dark:bg-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-neutral-500">
            ...
            <div class="flex justify-center">
                <button type="submit" class="inline-flex items-center gap-2 px-8 py-3 rounded-full bg-neutral-900 hover:bg-neutral-800 text-white text-lg font-semibold transition shadow">
        ...
        <div class="mt-5 flex flex-wrap justify-center gap-2 text-xs font-medium">
            <span class="px-3 py-1 rounded-full bg-white/80 dark:bg-gray-800 border border-neutral-300 dark:border-neutral-700 text-neutral-700 dark:text-neutral-300">Today's Registrations: {{ number_format($todayRegistrations) }}</span>
            <span class="px-3 py-1 rounded-full bg-white/80 dark:bg-gray-800 border border-neutral-300 dark:border-neutral-700 text-neutral-700 dark:text-neutral-300">Today's Households: {{ number_format($todayHouseholds) }}</span>
            <span class="px-3 py-1 rounded-full bg-white/80 dark:bg-gray-800 border border-neutral-300 dark:border-neutral-700 text-neutral-700 dark:text-neutral-300">Today's Shift Intake: {{ number_format($todayShiftPatients) }}</span>
        </div>
    </div>
</div>
...
<div class="space-y-4 mb-8">
    ...
    @elseif($searchBy === 'barcode' || $searchBy === 'nrc')
        <div class="rounded-xl border border-neutral-200 dark:border-neutral-700 bg-neutral-50/60 dark:bg-neutral-800/10 px-4 py-3">
            <h4 class="text-sm font-semibold text-neutral-900 dark:text-neutral-200">Patient Matches</h4>
        </div>
        @forelse($results as $row)
            ...
            @endphp
            <div class="rounded-2xl border border-neutral-300 dark:border-neutral-700 bg-white dark:bg-gray-800 shadow-sm p-4 md:p-5">
                <div class="grid grid-cols-1 md:grid-cols-12 gap-4 md:gap-5">
                    <div class="md:col-span-3">
                        <a href="{{ route('patients.show', ['ref' => $patientRef]) }}" class="text-4xl leading-tight font-bold text-neutral-900 dark:text-neutral-100 hover:underline">{{ $row->full_name ?: 'Unknown Patient' }}</a>
                    </div>
                ...
                <div class="mt-4 flex flex-wrap gap-2">
                    <a href="{{ route('patients.show', ['ref' => $patientRef]) }}" class="px-4 py-1.5 rounded-full bg-neutral-900 hover:bg-neutral-800 text-white text-xs font-semibold">Edit Profile</a>
                    <a href="{{ route('patients.show', ['ref' => $patientRef]) }}" class="px-4 py-1.5 rounded-full bg-neutral-700 hover:bg-neutral-600 text-white text-xs font-semibold">Attend to Patient</a>
                </div>
            </div>
        @empty
        ...
        <div class="rounded-xl border border-neutral-300 dark:border-neutral-700 bg-neutral-100/60 dark:bg-neutral-800/10 px-4 py-3 mt-2">
            <h4 class="text-sm font-semibold text-neutral-900 dark:text-neutral-200">Household Matches</h4>
        </div>
            ...
            <div class="rounded-2xl border border-neutral-300 dark:border-neutral-700 bg-white dark:bg-gray-800 shadow-sm p-4 md:p-5">
                <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
                    <div class="md:col-span-4">
                        ...
                        <p class="text-2xl font-bold text-neutral-900 dark:text-neutral-200">{{ $household->head_of_house ?: '—' }}</p>
                ...
                <div class="mt-4">
                    <a href="{{ route('households.show', ['ref' => $household->household_id]) }}" class="inline-flex px-4 py-1.5 rounded-full bg-neutral-900 hover:bg-neutral-800 text-white text-xs font-semibold">Open Household</a>
                </div>
            </div>
        @empty
            <div class="rounded-2xl border border-dashed border-slate-300 dark:border-slate-700 bg-white dark:bg-gray-800 p-6 text-center text-slate-500 dark:text-slate-400">
    ...
    @forelse($results as $row)
        ...
        <div class="rounded-2xl border border-neutral-300 dark:border-neutral-700 bg-white dark:bg-gray-800 shadow-sm p-4 md:p-5">
            ...
            <div class="mt-4 flex flex-wrap gap-2">
                <a href="{{ route('patients.show', ['ref' => $patientRef]) }}" class="px-4 py-1.5 rounded-full bg-neutral-900 hover:bg-neutral-800 text-white text-xs font-semibold">Edit Profile</a>
                <button type="button" class="px-4 py-1.5 rounded-full bg-neutral-700 hover:bg-neutral-600 text-white text-xs font-semibold">Admission</button>
                <button type="button" class="px-4 py-1.5 rounded-full bg-neutral-700 hover:bg-neutral-600 text-white text-xs font-semibold">Appointment</button>
                <button type="button" class="px-4 py-1.5 rounded-full bg-neutral-700 hover:bg-neutral-600 text-white text-xs font-semibold">Assign Queue</button>
                <button type="button" class="px-4 py-1.5 rounded-full bg-neutral-700 hover:bg-neutral-600 text-white text-xs font-semibold">Historical Visit</button>
                <button type="button" class="px-4 py-1.5 rounded-full bg-neutral-700 hover:bg-neutral-600 text-white text-xs font-semibold">Binding</button>
                <a href="{{ route('patients.show', ['ref' => $patientRef]) }}" class="px-4 py-1.5 rounded-full bg-neutral-500 hover:bg-neutral-400 text-white text-xs font-semibold">Attend to Patient</a>
            </div>
        </div>
    @empty
        <div class="rounded-2xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-gray-800 p-10 text-center">
            <p class="text-2xl font-semibold text-slate-400 dark:text-slate-500 italic mb-5">No Patient Found</p>
            <a href="{{ route('patients.create') }}" class="inline-flex items-center gap-2 px-6 py-2.5 rounded-full bg-neutral-900 hover:bg-neutral-800 text-white text-sm font-semibold transition shadow-sm">
