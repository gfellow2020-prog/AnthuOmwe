        ...
        return DB::transaction(function () use ($encounter, $data, $prescribedById, $screeningRecord): PharmacyPrescription {
            ...
            foreach ($data['items'] as $item) {
                PharmacyPrescriptionItem::create([
                    ...
                    'dose'                     => $item['dose']             ?? null,
                    'item_per_dose'            => isset($item['item_per_dose']) ? (int) $item['item_per_dose'] : null,
                    'frequency'                => $item['frequency']        ?? null,
                    'time_per'                 => $item['time_per']         ?? null,
                    'frequency_unit'           => $item['frequency_unit']   ?? null,
                    'duration'                 => $item['duration']         ?? null,
                    'duration_unit'            => $item['duration_unit']    ?? null,
                    'start_date'               => $item['start_date']       ?? null,
                    'end_date'                 => $item['end_date']         ?? null,
                    'quantity_prescribed'      => isset($item['quantity_prescribed']) ? (int) $item['quantity_prescribed'] : null,
                    'route'                    => $item['route']            ?? null,
                    'is_passer_by'             => (bool) ($item['is_passer_by'] ?? false),
